...
const getFastModel = () => getModel("gemini-2.0-flash-lite");
...
${summary.transcript}
  ...
  // Setup Gemini Live API for real-time transcription
  const setupGeminiLive = async (audioContext: AudioContext, stream: MediaStream) => {
    console.log("Setting up Gemini Live API for transcription");
    try {
      const ai = getAI();
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: (message: any) => {
            // Handle model's text response (output transcription)
            ...
            // Handle input audio transcription (if enabled in config)
            // Checking multiple possible field names based on different SDK versions/docs
            const inputTranscription = message.serverContent?.inputAudioTranscription?.text || 
                                     message.serverContent?.inputTranscription?.text;
            
            if (inputTranscription) {
              setLiveTranscript(prev => prev + inputTranscription + " ");
              liveTranscriptRef.current += inputTranscription + " ";
            }
            ...
            if (outputTranscription) {
              setLiveTranscript(prev => prev + outputTranscription + " ");
              liveTranscriptRef.current += outputTranscription + " ";
            }
          },
          onerror: (error: any) => {
        ...
        config: {
          ...
          systemInstruction: `You are a real-time professional transcriptionist and translator.
            ${language === 'auto' 
              ? "Auto-detect the spoken language dynamically." 
              : `The spoken language is ${language}.`}
            ${targetLanguage !== 'none' 
              ? `Translate everything exactly into ${targetLanguage}.` 
              : "Transcribe exactly what is said in the original language spoken."}
            Do not answer questions. Do not add commentary. Just output the text.`,
        }
      });

      geminiLiveSessionRef.current = sessionPromise;

      // Setup audio capture for Gemini Live
      ...
      console.error("Failed to setup Gemini Live:", error);
      setupWebSpeechFallback();
    }
  };
            ...
            if (event.results[i].isFinal) {
              ...
              if (languageRef.current === 'auto' && !detectedLanguageRef.current && liveTranscriptRef.current.trim().length > 15) {
                ...
                getAI().models.generateContent({
                  ...
                  const code = result.text?.trim().replace(/['"]/g, '');
